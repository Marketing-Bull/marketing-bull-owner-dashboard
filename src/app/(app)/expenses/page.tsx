...
function blankRecurring(categories: string[], defaultCategory: string | null): RecurringValues { return { description: "", vendor: "", amount: "", category: defaultCategory || categories[0] || "", company: "Marketing Bull", frequency: "monthly", dayOfMonth: "1", startDate: todayKey(), endDate: "", status: "active", paymentMethod: "", notes: "" }; }
...
function ExpenseForm({ formId, initial, clients, projects, accounts, categories, existingReceipt, uploading, onDirtyChange, onSubmit }:
  ...
  // A record saved under a category that was later renamed or deactivated keeps
  // its own label in the list, so editing it cannot silently recategorize it.
...
function RecurringForm({ formId, initial, categories, onDirtyChange, onSubmit }: { formId: string; initial: RecurringValues; categories: string[]; onDirtyChange: (dirty: boolean) => void; onSubmit: (values: RecurringValues) => void }) {
...
export default function ExpensesPage() {
  ...
  // Filters must reach every category that exists in the data, including ones
  // Settings has since retired; the form offers only what Settings still lists.
  const filterCategories = useMemo(
    () => Array.from(new Set([...categoryOptions, ...facets.categories.map((item) => item.value), ...recurring.map((item) => item.category)])).filter(Boolean).sort(),
    [categoryOptions, facets.categories, recurring]
  );
  ...
  return <TransactionPage>
    ...
    {tab === "entries" ? <>
      <FilterBar advancedOpen={advancedOpen} onToggleAdvanced={() => setAdvancedOpen((current) => !current)} activeFilters={activeFilters} onClear={() => setFilters((current) => ({ ...DEFAULT_FILTERS, pageSize: current.pageSize }))} advanced={<>
        ...
        <div className={styles.filterTabletOnly}><FilterField label="Category"><select className={styles.select} value={filters.category} onChange={(event) => patchFilters({ category: event.target.value })}><option value="">All categories</option>{filterCategories.map((category) => <option key={category} value={category}>{category}</option>)}</select></FilterField></div>
    ...
    <RecordSheet open={sheetMode !== null} title={tab === "recurring" ? (sheetMode === "edit" ? "Edit recurring expense" : "Add recurring expense") : (sheetMode === "edit" ? "Edit expense" : "Add expense")} subtitle={tab === "entries" && sheetMode === "create" && defaults ? "Using recent category, company, account, and payment values" : undefined} dirty={formDirty} onClose={() => setSheetMode(null)} footer={<>{tab === "entries" && sheetMode === "create" ? <button type="submit" form={formId} name="intent" value="another" className={styles.secondaryButton} disabled={busy || uploading}>Save & add another</button> : null}<button type="submit" form={formId} name="intent" value="close" className={styles.primaryButton} disabled={busy || uploading}>{uploading ? "Uploading receipt…" : busy ? "Saving…" : tab === "recurring" ? (sheetMode === "edit" ? "Save changes" : "Save recurring") : (sheetMode === "edit" ? "Save changes" : "Save expense")}</button></>}>
      ...
      {tab === "recurring" && recurringSeed ? <RecurringForm key={formKey} formId={formId} initial={recurringSeed} categories={categoryOptions} onDirtyChange={setFormDirty} onSubmit={async (values) => { const result = await mutate(editingRecurring ? "PUT" : "POST", editingRecurring ? `/api/expenses/recurring/${editingRecurring.id}` : "/api/expenses/recurring", recurringPayload(values)); if (result.success) { setFormDirty(false); setSheetMode(null); } }} /> : null}
    </RecordSheet>
